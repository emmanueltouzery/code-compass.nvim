...
local function get_definition_field_access_with_owner(ts_node, parent1, fieldOwner, run_finish, opts)
  ...
  get_all_superclasses(parent1, function(superclasses)
    ...
    helpers.run_and_parse_ast_grep(fieldName, candidates, opts, function(res) run_finish(res, opts) end)
  end)
end
...
local function run_finish(matches, opts)
  ...
  if #matches == 0 then
    -- not locals. maybe we're accessing a field?
    local ts_utils = require("nvim-treesitter.ts_utils")
    local ts_node = ts_utils.get_node_at_cursor()
    local parent1 = ts_node:parent()
    get_definition_field_access_with_owner(ts_node, parent1, "this", run_finish, opts)
    return
  end
...
local function get_definition_field_access(ts_node, parent1, opts)
  ...
  return get_definition_field_access_with_owner(ts_node, parent1, fieldOwner, run_finish, opts)
end
...
local function get_definition(opts)
  ...
  elseif parent1:type() == "method_invocation"
    ...
    if vim.fn.expand('<cword>'):sub(1, 1):match('[A-Z]') then
      ...
      -- try local variables
      local locals = locals.find_local_declarations()
      if #locals > 0 then
        run_finish(locals, opts)
      else
        -- not locals. maybe we're accessing a field?
        get_definition_field_access_with_owner(ts_node, parent1, "this", run_finish, opts)
      end
